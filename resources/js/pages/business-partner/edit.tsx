...
import { update } from '@/routes/business-partners';
...
export default function Edit({
    ...
    const [partnerableType, setPartnerableType] = useState(
        partner.partnerable_type ?? '',
    );
    const [partnerableId, setPartnerableId] = useState(
        partner.partnerable_id?.toString() ?? '',
    );
    const [receivableAccountId, setReceivableAccountId] = useState(
        partner.default_receivable_account_id?.toString() ?? '',
    );
    const [payableAccountId, setPayableAccountId] = useState(
        partner.default_payable_account_id?.toString() ?? '',
    );
        ...
        <AppLayout>
            ...
            <main className="p-4">
                <Heading
                    ...
                    description={`Updating details for ${partner.name}`}
                />

                <Form
                    action={update(partner.id)}
