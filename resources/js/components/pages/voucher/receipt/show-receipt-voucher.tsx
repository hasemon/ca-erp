...
import { ReceiptVoucherPDF } from '@/components/pages/voucher/receipt/receipt-voucher-pdf';
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
interface ShowReceiptVoucherProps {
    voucher: VoucherType;
    onEdit: () => void;
    onDeleted?: () => void;
}

export function ShowReceiptVoucher({
    voucher,
    onEdit,
    onDeleted,
}: ShowReceiptVoucherProps) {
    ...
    const printVoucher = async () => {
        await handlePrint({
            pdfDocument: <ReceiptVoucherPDF voucher={voucher} />,
            documentTitle: `Receipt_Voucher_${voucher.voucher_no}`,
    ...
    const handleDelete = () => {
        router.delete(receipt.destroy.url(voucher.id), {
            preserveScroll: true,
            onSuccess: () => {
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {
                        // Delete — only for draft vouchers
                        (voucher.status.value === 'draft' ||
                            (voucher.status.value === 'approved' &&
                                !voucher.is_locked)) && (
                            <>
