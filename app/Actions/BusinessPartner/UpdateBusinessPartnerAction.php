...
class UpdateBusinessPartnerAction
{
    public function __invoke(array $data, BusinessPartner $partner, ?Request $request = null): BusinessPartner
    {
        return DB::transaction(function () use ($data, $partner) {
            ...
            if (! empty($data['partnerable_type']) && ! empty($data['partnerable_id'])) {
                // Delete existing links for simplicity (one role only as per user preference)
                $partner->partnerables()->delete();
